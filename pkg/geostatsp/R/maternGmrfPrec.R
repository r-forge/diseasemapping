...
numbn <- structure(c(1L, 4L, 4L, 4L, 8L, 4L, 4L, 8L, 4L, 8L, 8L, 4L),
    ...
    ), names = "matSub"), class = "table"
)

ndist <- structure(c(
    0, 1, 1.4142135623731, 2, 2.23606797749979, 3, 2.82842712474619,
    3.16227766016838, 4, 3.60555127546399, 4.12310562561766, 5
), names = c(
    "1",
...
maternGmrfPrec.dgCMatrix <- function(N,
    ...
    if (identical(adjustEdges, FALSE)) {
        ...
        paramInfo$empirical <- data.frame(
            x =
                as.numeric(names(ev)),
            y = ev
        )
