...
<<swissgeostatsp, echo=FALSE, cache=TRUE>>=
data("swissRain")
...
swissRes =  lgm( formula=rain~ elevation, 
                     ...
                     aniso=TRUE )
A<-swissRes$summary[,c('estimate','ci0.1', 'ci0.9')]
@

<<swisssummarycompare, echo=FALSE>>=
table1 <- read.csv("Book1.csv")
colnames(table1) <- c('Parameter','Estimate','90% CI', 'Estimate','90% CI')
knitr::kable(table1, format = "latex", align = c("rcccc"),booktabs = T, linesep = "", caption= "Summary comparison", digits=3)%>%
 kable_styling(full_width = FALSE, position = "center", latex_options = "HOLD_position") %>%
 add_header_above(c(" ", "geostatsp" = 2, "gpuBatchMatrix" = 2)) 
@

<<swiss_profile_many,echo = FALSE, eval=TRUE, cache=TRUE, fig.width=4,fig.height=3, fig.show ="hold", fig.align='center', fig.cap = "profile LogL of slope parameter", out.width = "33%",dev='pdf',fig.pos="H">>=
#setwd("/home/ruoyong/diseasemapping/pkg/oldgpuRandom/inst/doc/L2021_11")
setwd("/home/ruoyong/L2021_11")
knitr::include_graphics(c('profile_logl_swiss.png','betaprofile.png','varprofile.png'))
@

<<swiss_profile_sole_prep, out.width = "50%", fig.align = "center",fig.pos="H",echo=FALSE>>=
knitr::include_graphics(c('swiss_pro_4.png','swiss_pro_2.png'))
@
...
<<meusedata, echo=FALSE, cache=TRUE>>=
data(meuse)
...
<<meuse_params, cache=TRUE, echo=FALSE>>=
  ParamList = list(
...
<<meuse_logl, cache=TRUE, message=FALSE>>=
result3<- gpuRandom::likfitLgmCov(spatialmodel=Res1,
                  params=params, 
                  boxcox=c(1,0,-0.903901505),  
                  paramToEstimate = c('range','shape','nugget','sdNugget', 'anisoRatio','anisoAngleRadians'), 
                  cilevel=0.95, 
                  type = "double",
                  reml=FALSE, 
                  NparamPerIter=400,
...
<<simulate_datasets, echo=FALSE, cache=TRUE, results='hide', message=FALSE>>=
...
<<ranuparams, echo=FALSE, cache=TRUE>>=
newParamListrn = list(
  range = c(exp(seq(log(0.3), log(1), len=10))*1000, exp(seq(log(1.2), log(2.9), len=12))*1000),  
  nugget = c(seq(0, 4, len=8), seq(4.5, 20, len=12)),  
  shape = 2,
  anisoRatio =1,
  anisoAngleRadians = 0.0
) 
paramsrn = do.call(expand.grid, newParamListrn)
@


<<run_forloop, echo=FALSE, results='hide', message=FALSE, warning=FALSE, cache=TRUE>>=
count = 0
breaksf = rep(0, 10)
makedata <- rep(0, 4)
estimates <- rep(0, 6)
...
for (i in 1:Nsim){
  ...
  makedata0 <- cbind(paramsrn[,1:2], result$LogLik, D=i)
...
<<graphs, echo = FALSE, eval=TRUE, cache=TRUE>>=
...
<<showgraphs, echo = FALSE, eval=TRUE, cache=TRUE, fig.width=4,fig.height=3, fig.show ="hold", fig.align='center', fig.cap = "range and nugget contours", out.width = "50%",dev='pdf'>>=
#par(mfrow = c(1, 2))
#forigin
knitr::include_graphics(c('p2Rplot_100.png','rsplot.png'))
@
