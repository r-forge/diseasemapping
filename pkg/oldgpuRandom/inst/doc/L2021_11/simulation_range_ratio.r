########################################################################################################################################
## ---gpuRandom------range vs ratio contour--------------------------------------------------------------
## set params
newParamListsn = list(
...
for (i in 1:15){
  result<-gpuRandom::likfitLgmCov2d(
    ...
    params=paramssn, 
    paramToEstimate = c('range','anisoRatio'),
    boxcox = 1,
    cilevel=0.8,
    type = "double",
    reml=FALSE, 
...
colnames(makedata) <- c('range','anisoRatio', 'LogLik', 'D')
...
library('ggplot2')
#rm(forigin_ns)
forigin_ns <- ggplot(subset(makedatahey, D==1), aes(x = range, y = anisoRatio, z=LogLik)) + theme_bw() + 
...
cols <- rainbow(15)
for (i in 2:15){
  forigin_ns <- forigin_ns + geom_contour(data = subset(makedatahey, D==i), aes(x = range, y = anisoRatio, z=LogLik), breaks=breaksf[i], col= cols[i])
}
forigin_ns
