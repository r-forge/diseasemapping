...
for (i in 1:15){
  result<-gpuRandom::likfitLgmCov2d(
    ...
    params=paramssn, 
    paramToEstimate = c('range','anisoAngleRadians'),
    boxcox = 1,
    cilevel=0.8,
    type = "double",
    reml=FALSE, 
...
colnames(makedata) <- c('range','anisoAngleRadians', 'LogLik', 'D')
...
library('ggplot2')
#rm(forigin_ns)
forigin_ns <- ggplot(subset(makedatahey, D==1), aes(x = range, y = anisoAngleRadians, z=LogLik)) + theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(size = 0.5, colour='grey') + 
  stat_contour(breaks=breaksf[1])  + 
  geom_vline(xintercept = trueParam['range'])+
  geom_hline(yintercept = trueParam['anisoAngleRadians'])

cols <- rainbow(15)
for (i in 2:15){
  forigin_ns <- forigin_ns + geom_contour(data = subset(makedatahey, D==i), aes(x = range, y = anisoAngleRadians, z=LogLik), breaks=breaksf[i], col= cols[i])
}
forigin_ns
...
# [1] 1
# > mean_estimates
# (Intercept)              cov1              cov2         sdSpatial             range anisoAngleRadians 
# 1.7512123         0.5329614         0.2651647         0.2751713      1048.0044129         0.1250333 
