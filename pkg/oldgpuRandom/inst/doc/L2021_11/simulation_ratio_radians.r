########################################################################################################################################
## ---gpuRandom------ratio vs radians contour--------------------------------------------------------------
## set params
newParamListsn = list(
...
for (i in 1:15){
  result<-gpuRandom::likfitLgmCov2d(
    ...
    params=paramssn, 
    paramToEstimate = c('anisoRatio','anisoAngleRadians'),
    boxcox = 1,
    cilevel=0.8,
    type = "double",
    reml=FALSE, 
...
colnames(makedata) <- c('anisoRatio','anisoAngleRadians', 'LogLik', 'D')
...
library('ggplot2')
#rm(forigin_ns)
forigin_ns <- ggplot(subset(makedatahey, D==1), aes(x = anisoRatio, y = anisoAngleRadians, z=LogLik)) + theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  geom_point(size = 0.5, colour='grey') + 
  stat_contour(breaks=breaksf[1])  + 
  geom_vline(xintercept = trueParam['anisoRatio'])+
  geom_hline(yintercept = trueParam['anisoAngleRadians'])

cols <- rainbow(15)
for (i in 2:15){
  forigin_ns <- forigin_ns + geom_contour(data = subset(makedatahey, D==i), aes(x = anisoRatio, y = anisoAngleRadians, z=LogLik), breaks=breaksf[i], col= cols[i])
}
forigin_ns
