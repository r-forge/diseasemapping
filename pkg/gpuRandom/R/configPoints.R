...
configMidPoint <- function(Model,
                           alpha=c(0.0005, 0.01, 0.1, 0.2, 0.5, 0.8, 0.95),  # a vector of confidence levels 1-alpha
                         Mle = NULL){
   ...
   #t(2.4478 *eig$vectors %*% diag(sqrt(eig$values)) %*%  t(pointsSphere4d24) + MleGamma)
   out_list <- list()
   ...
   if(length(Mle)==2){
      ...
      for(i in 1:length(alpha)){
         ...
         colnames(pointsEllipseGammaspace) <- names(MleGamma)
         #pointsEllipseGammaspace = t(2.4478 * eig$vectors %*% diag(sqrt(eig$values)) %*%  t(coords4d) + MleGamma)
         #plot(pointsEllipse)
...
#' @export
configEndPoint <- function(Model,
                         alpha=c(0.01, 0.1, 0.2,0.5, 0.8),
                         Mle = NULL# a vector of confidence levels 1-alpha
){
   
   ## get the Hessian
   Theorder <- c('range', 'shape', 'nugget', 'anisoRatio', 'anisoAngleRadians')
   parToLog <- c("range", "nugget")
   if(is.null(Mle)){
   Mle <- Model$optim$options[,'opt']
   }
   Mle <- Mle[order(match(names(Mle), Theorder))]
   ...
   names(MleGamma)[whichLogged] = paste("log(", parToLog[whichLogged], ")",sep="")
   ...
   ParamsetGamma <- matrix(MleGamma, nrow=nrow(derivGridDf), ncol=length(Mle), byrow=TRUE, dimnames = list(NULL, names(MleGamma))) + 0.01 * derivGridDf
   # temp <- ParamsetGamma[,'gamma3'] + 1i * ParamsetGamma[,'gamma4']
   # Paramset <- cbind(exp(ParamsetGamma[,paste("log(", parToLog, ")",sep="")]),Mod(temp) + 1,Arg(temp)/2)
   # colnames(Paramset) <- names(Mle)
   # toAdd = setdiff(c('range','shape','nugget','anisoRatio', 'anisoAngleRadians'), names(Mle))
   # otherParams = matrix(Model$parameters[toAdd], nrow=nrow(Paramset), ncol = length(toAdd),
   #                      dimnames = list(rownames(Paramset), toAdd), byrow=TRUE)
   ...
   if(length(Mle)==2){
      ...
      Paramset <- cbind(exp(ParamsetGamma[,paste("log(", parToLog, ")",sep="")]), 
                        ParamsetGamma[,'gamma3']^2 + ParamsetGamma[,'gamma4']^2 + 1, atan(ParamsetGamma[,'gamma4']/ParamsetGamma[,'gamma3'])/2)
   }
   colnames(Paramset) <- names(Mle)  
   toAdd = setdiff(c('range','shape','nugget','anisoRatio', 'anisoAngleRadians'), names(Mle))
   otherParams = matrix(Model$parameters[toAdd], nrow=nrow(Paramset), ncol = length(toAdd),
   ...
   if(length(Mle)==2){
      ...
      HessianMat[1,2] <- (-3*A[6] + 4*A[7] - A[8] + 3*A[9] - 4*A[10] + A[11])/(4*0.01^2)
      HessianMat[2,1] <- HessianMat[1,2]
   }else if(length(Mle)==4){
      HessianMat[1,1] <- (A[1] + A[2] -2*Origin)/(0.01^2)
      HessianMat[2,2] <- (2*Origin - 5*A[3] + 4*A[4] - A[5])/(0.01^2)
      ...
      HessianMat[3,3] <- (A[5] + A[6] -2*Origin)/(0.01^2)
      HessianMat[4,4] <- (A[7] + A[8] -2*Origin)/(0.01^2)
      HessianMat[5,5] <- (A[9] + A[10] -2*Origin)/(0.01^2)
      
      HessianMat[1,2] <- (A[9] - A[10] - A[11] + A[12])/(4*0.01^2)
      HessianMat[1,3] <- (A[13] - A[14] - A[15] + A[16])/(4*0.01^2)
      HessianMat[1,4] <- (A[17] - A[18] - A[19] + A[20])/(4*0.01^2)
      HessianMat[1,5] <- (A[21] - A[22] - A[23] + A[24])/(4*0.01^2)
      HessianMat[2,3] <- (A[25] - A[26] - A[27] + A[28])/(4*0.01^2)
      HessianMat[2,4] <- (A[29] - A[30] - A[31] + A[32])/(4*0.01^2)
      HessianMat[2,5] <- (A[33] - A[34] - A[35] + A[36])/(4*0.01^2)
      HessianMat[3,4] <- (A[37] - A[38] - A[39] + A[40])/(4*0.01^2)
      HessianMat[3,5] <- (A[41] - A[42] - A[43] + A[44])/(4*0.01^2)
      HessianMat[4,5] <- (A[45] - A[46] - A[47] + A[48])/(4*0.01^2)
   ...
   if(length(Mle)==2){
      pointsSphere = exp(1i*seq(0, 2*pi, len=30))
      pointsSphere2d = cbind(Re(pointsSphere), Im(pointsSphere))
      for(i in 1:length(alpha)){
         clevel <- stats::qchisq(1 - alpha[i], df = 2)
         pointsEllipseGammaspace = t(sqrt(clevel) * eig$vectors %*% diag(sqrt(eig$values)) %*%  t(pointsSphere2d) + MleGamma)
         colnames(pointsEllipseGammaspace) <- names(MleGamma)
         #pointsEllipseGammaspace = t(2.4478 * eig$vectors %*% diag(sqrt(eig$values)) %*%  t(pointsSphere4d24) + MleGamma)
         #plot(pointsEllipse)
         pointsEllipse <- cbind(exp(pointsEllipseGammaspace[,paste("log(", parToLog, ")",sep="")]))
         colnames(pointsEllipse) <- names(Mle)
         out_list[[i]] = pointsEllipse
      }   
   }else if(length(Mle)==4){
   load('/home/ruoyong/diseasemapping/pkg/gpuRandom/data/shpere4d24.RData')
   for(i in 1:length(alpha)){
      clevel <- stats::qchisq(1 - alpha[i], df = 4)
      pointsEllipseGammaspace = t(sqrt(clevel) * eig$vectors %*% diag(sqrt(eig$values)) %*%  t(pointsSphere4d24) + MleGamma)
      colnames(pointsEllipseGammaspace) <- names(MleGamma)
      #pointsEllipseGammaspace = t(2.4478 * eig$vectors %*% diag(sqrt(eig$values)) %*%  t(pointsSphere4d24) + MleGamma)
      #plot(pointsEllipse)
      temp <- pointsEllipseGammaspace[,'gamma3'] + 1i * pointsEllipseGammaspace[,'gamma4']
      pointsEllipse <- cbind(exp(pointsEllipseGammaspace[,paste("log(", parToLog, ")",sep="")]), Mod(temp) + 1, Arg(temp)/2)
      colnames(pointsEllipse) <- names(Mle)
      out_list[[i]] = pointsEllipse
   }
   }else if(length(Mle)==5){
      load('/home/ruoyong/diseasemapping/pkg/gpuRandom/data/shpere5d10.RData')
      for(i in 1:length(alpha)){
         clevel <- stats::qchisq(1 - alpha[i], df = 5)
         pointsEllipseGammaspace = t(sqrt(clevel) * eig$vectors %*% diag(sqrt(eig$values)) %*%  t(pointsSphere5d10) + MleGamma)
         colnames(pointsEllipseGammaspace) <- names(MleGamma)
         temp <- pointsEllipseGammaspace[,'gamma3'] + 1i * pointsEllipseGammaspace[,'gamma4']
         pointsEllipse <- cbind(exp(pointsEllipseGammaspace[,paste("log(", parToLog, ")",sep="")]), Mod(temp) + 1, Arg(temp)/2)
         colnames(pointsEllipse) <- names(Mle)
