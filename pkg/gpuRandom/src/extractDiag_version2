...
#define DEBUGKERNEL
template <typename T> 
std::string extract_some_diag_string2(int Ndatasets,    // number of Y's
                                     int NpadColY, 
                                     int NpadColYX,
  ...
  result += 
    "\n__kernel void extract_some_diag2(\n"
...
void extractDiag_version2(
  ...
  std::string extractSomeDiagKernelString = extract_some_diag_string2<double>(//NparamPerIter[0],  
                 Ndatasets,
                 ssqY.internal_size2(), 
  ...
  viennacl::ocl::kernel &extractSomeDiagKernel = my_prog.get_kernel("extract_some_diag2");
  ...
  //viennacl::ocl::command_queue theQueue = extractSomeDiagKernel.context().get_queue();
...
// [[Rcpp::export]]
void extractDiagBackend_version2(
    Rcpp::S4 ssqYXR,// viennacl::vector_base<int>  rowSum, viennacl::vector_base<int>  colSum,  
    Rcpp::S4 ssqYR,
    Rcpp::IntegerVector NparamPerIter,
    int Ndatasets,
  ...
  extractDiagTemplated2(ssqYXR,ssqYR, NparamPerIter,Ndatasets,numWorkItems);
