...
myParamsBatch = rbind(c(shape=1.25, range=5, variance = 1.5, nugget = 0, anisoRatio = 1, anisoAngleRadians = 0))
...
#var4cpu[1:12,1:7]
dim(var4cpu)
#outputBatchF[1:25,]-var4cpu
range(as.matrix(outputBatchF)-var4cpu)
```
...
chol4gpu = as.matrix(outputBatchF)
chol4gpu[upper.tri(chol4gpu)] = 0
diag(chol4gpu) = 1


range(cholCpu - chol4gpu)
range(theDiag - as.matrix(diagMat))

(cholCpu - chol4gpu)[30:60,]



theDiag[31]
as.matrix(diagMat)[31]
which((theDiag - as.matrix(diagMat))>0.1)
```
