...
print(quantile(fromCpu - as.matrix(outputBatchF)[
  seq(from=1+ncol(outputBatchF) * (D-1), len=ncol(outputBatchF)),], na.rm=TRUE))
}
```
...
outputChol2 = deepcopy(outputBatchF)
diagMat2 = deepcopy(diagMat)
...
gpuRandom::cholBatch(
  outputChol2, diagMat2, numbatchD=nrow(myParamsBatch),
  Nglobal = NglobalChol,
  Nlocal = NlocalChol,
  NlocalCache = 20)

stuff = as.matrix(diagMat - diagMat2)
range(as.matrix(outputChol - outputChol2))

as.matrix(outputBatchF[1:12,1:7])
as.matrix(outputChol[1:12,1:7])
as.matrix(diagMat[,1:7])
```

```{r cholCheck, include=FALSE, eval=F}
...
recovered = chol4gpu %*% diag(as.vector(diagMat[D,])) %*% t(chol4gpu)
print(quantile(recovered - as.matrix(outputBatchF[
  seq(1+(D-1)*nrow(coordsGpu), len=nrow(coordsGpu)),]),
  prob = c(0, 0.01, 0.05, 0.95, 0.99, 1)))
}
```
...
xx = matrix(-99.9, ncol(outputChol), nrow(outputChol))
gpuRandom:::copyToCpu(xx, outputChol)
xx[1:5,1:5]
as.matrix(outputChol)[1:5,1:5]
```
