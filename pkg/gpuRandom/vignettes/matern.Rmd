...
#outputChol2 = vclMatrix(diag(ncol(outputChol))[rep(1:ncol(outputChol), nrow(diagMat)),], type='double')    
#diagMat2 = vclMatrix(matrix(0,#1:prod(dim(diagMat)),
#                            nrow(diagMat), ncol(diagMat)), type='double')
#zMatGpu2 = vclMatrix(matrix(1, nrow(zMatGpu), ncol(zMatGpu)), type='double')
...
gpuRandom:::multiplyLowerDiagonalBatchBackend(
  ...
  Nglobal = c(1,1,16),#c(32,32,4),#c(64,8,8), # rows, columns, matrix
  Nlocal = c(1,1,1),#c(8,8,1),  # anything, anything 1
  NlocalCache = 5000)
...
plot(apply(resGpu - resCpu, 1, sum)[1:800])
