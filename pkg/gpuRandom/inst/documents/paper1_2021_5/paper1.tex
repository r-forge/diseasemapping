...
R> library("clrng")
R> setBaseCreator(c(111, 222, 333, 444, 555, 666))
R> myStreamsCpu <- createStreamsCpu(4)
R> t(myStreamsCpu)
\end{CodeInput}
\begin{CodeOutput}
##              [,1]       [,2]       [,3]       [,4]
## current.g1.1  111  272931565 1097663848  628119774
## current.g1.2  222  750526866  712444468  211319804
## current.g1.3  333 1340941094  522805545  147140763
## current.g2.1  444 1101709871  362320065 1665200176
## current.g2.2  555 1485736734 1950619531  892386138
## current.g2.3  666  375100068 1839101248  734766081
## initial.g1.1  111  272931565 1097663848  628119774
## initial.g1.2  222  750526866  712444468  211319804
## initial.g1.3  333 1340941094  522805545  147140763
## initial.g2.1  444 1101709871  362320065 1665200176
## initial.g2.2  555 1485736734 1950619531  892386138
## initial.g2.3  666  375100068 1839101248  734766081
\end{CodeOutput} 
\end{CodeChunk} 
...
\begin{CodeChunk}
\begin{CodeInput}
R> myStreamsGpu2 = createStreamsGpu(4)
\end{CodeInput} 
\end{CodeChunk} 
...
There are a few important notes on the usage of these functions. (1), Users should run the function \fct{setBaseCreator} once at the very beginning of a \proglang{R} session if they would like to choose their own initial seeds. If \fct{setBaseCreator} is not called before \fct{createStreamsCpu} or \fct{createStreamsGpu}, then streams are created using the package's default initial seed. (2), Users may choose either \fct{createStreamsCpu} or \fct{createStreamsGpu} to run and must stick with that choice for their entire program, switching between \fct{createStreamsCpu} and \fct{createStreamsGpu} in the same program will create some of the streams twice.  When you create a new stream, it should not have the same seed as another existing stream, just like when you create a new car, it must have a distinct serial number. Streams should be created only once, and moved or copied if needed. 
...
const int startvalue = (get_global_id(0) + get_global_size(0) * get_global_id(1)) * NpadStreams;
...
\end{CodeInput}
\begin{CodeOutput}
## [1] 0.417 0.298 0.193 0.778 0.953 0.856
\end{CodeOutput} 
\end{CodeChunk} 
...
\begin{CodeChunk}
\begin{CodeInput}
R> streams <- createStreamsGpu(4)
R> r_matrix <- clrng::rexp(c(2, 4), rate = 1, streams, Nglobal = c(2,
+    2), type = "double")
R> as.matrix(r_matrix)
\end{CodeInput}
\begin{CodeOutput}
##       [,1]   [,2] [,3]    [,4]
## [1,] 0.479 0.6886  1.2 0.62493
## [2,] 0.721 0.0265  2.3 0.00598
\end{CodeOutput} 
\end{CodeChunk} 
