...
\begin{verbatim}
as.vector(
  clrng::runif(n=6, streams=myStreamsGpu, Nglobal=c(2,2), type = "double"))
...
Below is an example that we produce a matrix of exponential random number with expectation 1.
\begin{knitrout}
\definecolor{shadecolor}{rgb}{1, 1, 1}\color{fgcolor}\begin{kframe}
\begin{verbatim}
...
as.matrix(r_matrix)
##           [,1]        [,2]      [,3]      [,4]
## [1,] 0.2627643 0.005952261 0.4535731 0.1863516
## [2,] 0.5783051 0.241706604 1.2820227 0.4889530
\end{verbatim}
\end{kframe}
\end{knitrout}
...
myRaster = geostatsp::squareRaster(swissBorder, Ngrid)
myRaster
## class      : RasterLayer 
## dimensions : 51, 80, 4080  (nrow, ncol, ncell)
## resolution : 4358.065, 4358.065  (x, y)
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in nrow(param): argument "{}param"{} is missing, with no default}}\begin{verbatim}
dim(maternCov)
\end{verbatim}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'maternCov' not found}}\end{kframe}
\end{knitrout}
...
diagMat = gpuBatchMatrix::cholBatch(
  maternCov,
#                          D=diagMat, 
#                          numbatchD=nrow(params),
                          Nglobal= c(128, 8), Nlocal= c(32, 8)
                        )
\end{verbatim}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in nrow(A): object 'maternCov' not found}}\end{kframe}
\end{knitrout}
\begin{gather}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'nrow': object 'maternCov' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in gpuBatchMatrix:::multiplyLowerDiagonalBatchBackend(output, L, : object 'simMat' not found}}\end{kframe}
\end{knitrout}
\begin{gather}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in h(simpleError(msg, call)): error in evaluating the argument 'x' in selecting a method for function 'nrow': object 'paramsGpu' not found}}\end{kframe}
\end{knitrout}
