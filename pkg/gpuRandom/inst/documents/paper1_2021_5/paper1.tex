...
There are a few important notes on the usage of these above functions. (1), Users should run \fct{setBaseCreator} once at the very beginning of a \proglang{R} session if they would like to choose their own initial seeds. If \fct{setBaseCreator} is not called before \fct{createStreamsCpu} or \fct{createStreamsGpu}, then streams take the package's default initial seed. (2), Users may choose either \fct{createStreamsCpu} or \fct{createStreamsGpu} to create streams and must stick with that choice for their entire program, switching between \fct{createStreamsCpu} and \fct{createStreamsGpu} in the same program is very likely to create some streams twice.  A newly created stream should not have the same seed as another existing stream, just like when you create a new car, it must have a distinct serial number. Streams should be created only once, and moved or copied if needed. And also it causes unnecessary data transfer between host and device if users switch often between \fct{createStreamsCpu} and \fct{createStreamsGpu}.
...
\begin{CodeChunk}
\begin{CodeInput}
R> r_matrix <- clrng::rexp(c(2, 4), rate = 1, myStreamsGpu2,
+    Nglobal = c(2, 2), type = "double")
R> as.matrix(r_matrix)
\end{CodeInput}
\begin{CodeOutput}
##       [,1] [,2] [,3]  [,4]
## [1,] 0.433 2.20 1.00 0.689
## [2,] 1.770 1.13 1.48 1.205
\end{CodeOutput} 
\end{CodeChunk} 
