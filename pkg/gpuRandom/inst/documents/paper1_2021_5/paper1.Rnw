...
\end{itemize}
\fct{createStreamsGpu} is adapted from the clrngMrg31k3pCreateStreams() and creates streams directly on a GPU device. We can have streams on GPU by either converting the \code{myStreamsCpu} to a `\code{vclMatrix}' or producing streams using \fct{createStreamsGpu}. Setting a seed same as the MRG31k3p default seed in \fct{createStreamsGpu}, we can produce on device exactly the same streams as those created on host as follows,
<<createStreams_Gpu>>=
...
\begin{figure}[H]
\centering
<<largenumberuniforms, eval=FALSE, echo=FALSE, fig.height=4, fig.width=4, fig.show='hold', dev='png'>>=
...
\itemsep0em 
  ...
  \item \code{type}: ``double'' or ``float'' or returned test statistics.
...
\subsection{Comparing run-time: Month data example}
...
\subsection{Comparing run-time: Week data example}
...
@
\caption{Frequency distribution for test statistics from month and week data tables. Figure \ref{fig4} visualizes the approximate sampling distributions of the test statistics from the two examples "Month" and "Week", by plotting a histogram for each of the example. The values of observed statistics are calculated by \fct{gpuRandom::logfactSum} and are marked with a blue line on each plot. \label{fig4}}
\end{figure}
...
<<histAndQQ_prepare,eval=FALSE,cache=TRUE>>=
streams <- createStreamsGpu(n =512*128)
...
\begin{figure}[H]
\centering
 <<histAndQQ, echo=TRUE, eval=FALSE, cache=TRUE, fig.height=4, fig.width=4, fig.show='hold',dev='png'>>=
