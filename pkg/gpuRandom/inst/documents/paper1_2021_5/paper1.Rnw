...
<<simStuff2, eval=TRUE,tidy=FALSE>>=
...
library(knitr)
knitr::kable(month,format="latex", align = c("rrrrrrrrrrrr"), 
...
knitr::kable(week,format="latex",align = c("rrrrrrrrrrrr"), caption = "Day-of-week birth anomaly data\\label{tab:week}", booktabs=TRUE,linesep = "") %>%
  kable_styling(full_width = F, position = "center", latex_options = "HOLD_position")
@
...
hist(result_week$cpu, xlab=TeX('-$\\sum(\\log(n_{ij}!))$'), ylab="Frequency", main="Week data")
...
params = 
rbind(c(shape=1.25, range=0.5*1e04, variance = 1.5, nugget = 0,anisoRatio = 1, anisoAngleRadians = 0), 
c(shape=2.15, range=0.25*1e04, variance = 2, nugget = 0, anisoRatio = 4, anisoAngleRadians = pi/7),
c(shape=0.55, range=1.5*1e04, variance = 2, nugget = 0, anisoRatio = 4, anisoAngleRadians = pi/7),
c(shape=2.15, range=0.5*1e04, variance = 2, nugget = 0, anisoRatio = 4, anisoAngleRadians = -pi/7),
c(shape=2.15, range=0.5*1e04, variance = 2, nugget = 0, anisoRatio = 2, anisoAngleRadians = pi/4))
@
...
<<maternplot, echo=FALSE, eval=TRUE, cache=FALSE,dev='png', out.width = '0.45\\textwidth', fig.height=3, fig.width=6, fig.cap="Simulated Gaussian random fields", fig.subcap = names(simRaster)>>=
...
<<maternplot, echo=TRUE, eval=FALSE>>=
@
