...
\end{itemize}
Next we show that by using \code{myStreamsGpu2} which is the same as \code{myStreamsGpu1},  \fct{runif} returns the same sequence of random numbers as \code{sim_1}. 
<<eval=TRUE, tidy=TRUE>>=
