...
library(knitr)
knitr::kable(head(month),format="latex", align = c("rrrrrrrrrrrr"), caption = "Month\\label{tab:month}", booktabs=TRUE,linesep = "") %>%
...
knitr::kable(week,format="latex",align = c("rrrrrrrrrrrr"), caption = "Week\\label{tab:week}", booktabs=TRUE,linesep = "") %>%
  kable_styling(full_width = F, position = "center", latex_options = "HOLD_position")
@
...
dim(maternCov)
gpuBatchMatrix::maternBatch(var=maternCov, coords=coordsGpu, 
                            param=paramsGpu,
...
gpuBatchMatrix::cholBatch(A=maternCov, 
                          D=diagMat, 
