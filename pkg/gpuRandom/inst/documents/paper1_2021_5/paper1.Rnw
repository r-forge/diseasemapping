...
\pkg{clRNG} is an \proglang{OpenCL} library for uniform random number generation, it provides four different RNGs: the MRG31k3p, MRG32k3a, LFSR113, and Philox-4×32-10 generators. \pkg{gpuRandom} package uses the MRG31k3p RNG from the \proglang{clRNG}, making it able to generate random numbers on GPUs. In what follows, we will illustrate how to create streams and how to use streams to generate (standard) uniform and (standard) Gaussian random numbers. 
