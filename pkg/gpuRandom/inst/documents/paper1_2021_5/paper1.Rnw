...
matrix(as.matrix(myStreamsGpu2), nrow(myStreamsCpu), ncol(myStreamsCpu), dimnames = dimnames(myStreamsCpu))
range(as.matrix(myStreamsGpu2) - myStreamsCpu)
@
...
<<eval=TRUE, tidy=TRUE>>=
myStreamsGpu3 = deepcopy(myStreamsGpu2)
sim_1 = runif(n=6,streams=myStreamsGpu2, Nglobal=c(1,4), type="double")
as.vector(sim_1)
@
...
\end{itemize}
Next we show that by using a stream \code{myStreamsGpu3} same as \code{myStreamsGpu2},  \fct{runif} returns a vector \code{sim_2} of same random numbers as \code{sim_1}. 
<<eval=TRUE,tidy=TRUE>>=
sim_2 <-runif(n=6, streams=myStreamsGpu3, Nglobal=c(1,4), type = "double")
as.vector(sim_2)
@
Notice that \code{myStreamsGpu2} is a \code{deepcopy} of \code{myStreamsGpu2} before \code{myStreamsGpu2} is used. Reusing \code{myStreamsGpu2} will produce a vector different from \code{sim_1} (see \code{sim_3}). Because each time a random number is generated, the current state of the stream is changed (advances by one position), below shows the first six rows (current states) of \code{myStreamsGpu2} after it being used.
<<eval=TRUE,tidy=TRUE>>=
sim_3 = runif(n=6,streams=myStreamsGpu2, Nglobal=c(1,4), type="double")
as.vector(sim_3)
t(as.matrix(myStreamsGpu2))[1:6,]
@
