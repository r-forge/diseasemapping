...
<<deviceinfo, echo=FALSE,cache=TRUE>>=
library(kableExtra)
a <- data.frame(
...
knitr::kable(dt, col.names = NULL, caption = "Summary of comparions of Fisher's test simulation on different devices.\\label{tab:summary}") %>%
  kable_styling(full_width = FALSE, position = "center",latex_options = "HOLD_position") %>%
