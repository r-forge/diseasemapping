...
In \proglang{clRNG} library, a stream object is a structure that contains three states (see the \proglang{R} output below): the current state of the stream, the initial state of the stream (or seed), and the initial state of the current substream. The \proglang{clRNG} function \fct{clrngMrg31k3pCreateStreams} creates streams on the host computer using the MRG31k3p RNG. If we want to use the streams in work items on a GPU device, the streams have to be copied from the host to the global memory of the GPU device, and then each work item picks streams from the global memory and copies their current states only to its private memory. In \pkg{gpuRandom}, to avoid unnecessary data transfer between host and device, we are able to create streams both on the host computer and on the GPU device. \fct{gpuRandom::CreateStreamsCpu} is an \proglang{R} interface function of \fct{clrngMrg31k3pCreateStreams} that creates streams on the host as \proglang{R} matrices, so that users can easily view the streams. \fct{gpuRandom::CreateStreamsGpu} is adapted from \fct{clrngMrg31k3pCreateStreams} with the advantage of creating streams directly on the GPU device. From our experience, transferring a large-size matrix of streams from host to device by converting it to a `\code{vclMatrix}' object can take very long time, hence, \fct{gpuRandom::CreateStreamsGpu} is more efficient and more frequently used. Below, we show the usage of these two functions by generating 4 streams on the host and on the device respectively.
<<streams on CPU>>=
# Create streams on host
streams_on_CPU<-gpuRandom::CreateStreamsCpu(Nstreams=4)
t(streams_on_CPU)
@
\fct{gpuRandom::CreateStreamsCpu} has only one argument \code{Nstreams}, which is the number of streams to create. For the MRG31k3p RNG, each state of the stream object is comprised of six 31-bit integers, the state of the first stream (seed of the \proglang{clRNG} library) has a default value ``12345''. By setting a seed identical to the MRG31k3p default seed in \fct{gpuRandom::CreateStreamsGpu}, we can produce on device exactly the same streams as the streams created on host as follows,
...
\end{itemize}
Streams in \pkg{gpuRandom} are like \fct{.Random.seed} in \proglang{R}. By setting the RNG status with the \fct{set.seed} function in \proglang{R}, we are able to repeat the sequence of random seeds, which is an important quality criteria of RNGs. Similarly in \pkg{gpuRandom}, with the \code{seedR} argument, we are able to produce exactly the same sequence of streams as many times as we want.


\subsection{Using streams to generate  i.i.d. $U (0, 1)$ random numbers on GPU}
The streams created on GPU are then used in work items that executes in parallel on GPU device, in \pkg{gpuRandom} each work item takes one distinct stream to generate random numbers, so number of streams should equal to the number of total work items in use. A part of the kernel (\proglang{OpenCL} functions for execution on the device) for generating uniform random numbers is shown below. Kernels are written in \proglang{OpenCL C} language, \code{__kernel} declares a function as a kernel, pointer kernel arguments must be declared with an address space qualifier, for example \code{__global} and \code{__local}. Pointers to the \code{streams} and to the matrix \code{out} on the global memory are passed to the kernel as arguments. \code{Nrow} and \code{Ncol} represent number of rows and columns of the matrix \code{out} respectively. \code{NpadCol} is the internal number of columns of \code{out}, \code{NpadStreams} is internal number of columns of the \code{streams} matrix (these variables are defined in macros not shown here). Each stream is taken to the private memory of each work item by the function \fct{streamsToPrivate}. Here we use a 2-dimensional index space with work-group size $1 \times 2$.  \code{index} represents the position of the work item when mapping it to a one-dimensional vector. The function \fct{clrngMrg31k3pNextState} gives a uniform random integer named \code{temp} between 0 and 4294967295,  the value of \code{temp} is then scaled to be in the interval $(0,1)$ by multiplying it by a constant \code{mrg31k3p\_NORM\_cl}, which is defined to be $1/4294967295$ in macro not shown here. At the end of generating random numbers, streams are transferred back to global memory through the function \fct{streamsFromPrivate}.
\begin{framed}
...
\begin{figure}[H]%
    ...
    \caption{name?\label{fig1}}%
\end{figure}

In the following code, we use the 4 streams created in Section 2.1 and 4 work items to generate a vector \code{sim_1} of 6 i.i.d. $U (0,1)$ random numbers by the function \fct{gpuRandom::runif}. By default \code{Nglobal} is set as (64,8), and a matrix of \code{prod(Nglobal)} streams created on GPU is supplied. The output is an S4 object (a `\code{vclVector}' or a `\code{vclMatrix}'), to view the generated random numbers, we need to convert it to an R vector or matrix, in this way, the random numbers are moved from the global memory of device to the host computer.
<<eval=TRUE, tidy=TRUE>>=
...
\begin{itemize}
  \item \code{n}: a number specifying the length if to create a vector, or a vector of length 2 specifying the row and column number if to create a matrix.
  \item \code{streams}: streams used for random number generating.
...
\end{itemize}
Next we show that using the same streams (\code{streams1} and \code{streams2}),  \fct{gpuRandom::runif} returns a vector \code{sim_2} of same random numbers as \code{sim_1}. 
<<eval=TRUE,tidy=TRUE>>=
sim_2 <-gpuRandom::runif(n=6, streams=streams2, Nglobal=c(1,4), type = "double")
as.vector(sim_2)
@
Note that \code{streams2} is a \code{deepcopy} of \code{streams1} before \code{streams1} is used, reusing the \code{streams1} will produce a vector  different from \code{sim_1} (result of \code{sim_3}), because each time a random number is generated, the current state of the stream is changed (advances by one position). See below the first six rows (current states) of \code{streams1} after it being used.
<<eval=TRUE,tidy=TRUE>>=
sim_3 = gpuRandom::runif(n=6,streams=streams1, Nglobal=c(1,4), type="double")
as.vector(sim_3)
t(as.matrix(streams1))[1:6,]
@
In order to reproduce the same sequence of random numbers, we can save the streams in a file on CPU, because GPU device does not keep memory for streams objects after each execution. Below is the \proglang{R} code in which we use the saved streams \code{streams_from_GPU.rds} to produce a vector \code{sim_saved}, which is also same as \code{sim_1}.
...
\end{lstlisting}
\end{framed}
The kernel is adjusted from the kernel for generating uniform random numbers in \ref{uniform}, we only add the part that does the Box-Muller transformation. If developers want to use other Gaussian transform methods, this kernel code is easy to be modified and used in other \proglang{R} packages.  In the code, \code{part0} and \code{part1} correspond to $R$ and $\Theta$ in the formulas respectively. \code{barrier(CLK_LOCAL_MEM_FENCE)} ensures correct ordering of memory operations to local memory, so that $(X_1,Y_1), (X_2,Y_2), \dots, (X_n,Y_n)$ are generated correctly in pairs, errors such as $(X_n, Y_{(n-1)})$ or $(X_{(n-1)}, Y_{(n)})$ will not occur. We illustrate the matrix filling process with the previous example: to create a $4 \times 6$ matrix ((b) in Figure \ref{fig3}) of Gaussian random numbers using a $2 \times 4$ NDRange ((a) in Figure \ref{fig3}) and 8 streams. 
...
\begin{figure}[H]%
    ...
    \label{fig3}
\end{figure}
As Gaussian numbers are generated in pairs using the Box-Muller method, work items are organized to be $1 \times 2$ work groups in the index space (a), the shading in the grids of (a) distinguishes work groups. Each work group generates a pair of Gaussian random numbers \code{(X,Y)} in each iteration, where the work item of `\code{get_local_id(1)}$=0$' generates the \code{X} in a pair, and work item of `\code{get_local_id(1)}$=1$' generates the \code{Y} in a pair. The numbers in the third row in each gird of (a) represents the local index of each work item. Cells of the matrix are filled from left to right, that is, in the first iteration of execution, the 8 work items generate random numbers in the leftmost two columns (8 cells) of matrix simultaneously, the second iteration fills random numbers in the middle two columns of the matrix, and the third iteration  fills the rest two columns of the matrix simultaneously. Therefore, each work item generates one Gaussian random number in each iteration, the filling process is the same as the filling process of Figure \ref{fig1}.  

We show the usage of \fct{gpuRandom::rnorm} by generating a $512 \times 1024$ matrix named \code{normal_matrix} of double presicion Gaussian random numbers, and assess Normality by making a histogram (left panel of Figure \ref{fig2}) and a Q-Q plot (right panel of Figure \ref{fig2}) of the generated random numbers. The points in the Q-Q plot in Figure \ref{fig2} fall along the diagonal line, we have good evidence the data are from Gaussian distribution. We find doing a Q-Q plot can occupy a large amount of time in the process of producing an R markdown file, so as an incidental work, we make the function \fct{gpuRandom::qqnorm} that does the quantile computation for the normal distribution part (adapted from \fct{stat::qnorm}) on GPU, which speeds up the Q-Q plot greatly.
...
@
\caption{\label{fig2} Histogram and Q-Q plot of the GPU-generated random numbers.}
\end{figure}
Let's compare the time of generating a large-size matrix of more than 26.8 million Gaussian random numbers of double precision using the \fct{stats::rnorm} and using the \fct{gpuRandom::rnorm}. Both process are not time-consuming, however, using \fct{gpuRandom::rnorm} with $128 \times 128$ work-items is about 10 times (the number may fluctuate a bit from executions) faster than using the \fct{stats::rnorm}. The difference in elapsed time becomes larger when matrix size goes larger.
<<random normal time compare, eval=TRUE, cache=FALSE>>=
system.time(gpuRandom::rnorm(c(16384,16384), streams=streams, 
                             Nglobal=c(512,128), type="double"))
system.time(matrix(stats::rnorm(16384*16384),16384,16384))
@
...
library(knitr)
library(kableExtra)
knitr::kable(head(month, n=12),format="latex",  caption = "Month\\label{tab:month}") %>%
...
@
\fct{gpuRandom::fisher.sim} takes about 2.3 seconds, compared to the elapsed time of \fct{gpuRandom::fisher.sim},  \fct{gpuRandom::fisher.sim} takes less than a quater of the time. 
<<timecomparemonth3Gpu, eval=TRUE,cache=TRUE>>=
...
<<Timecompareweek, echo=FALSE, eval=TRUE, results="hide">>=
library(latex2exp)
data<-read.csv("/home/ruoyong/paper2/weekday.csv") 
...
knitr::kable(head(week, n=7),format="latex",caption = "Week\\label{tab:week}") %>%
  kable_styling(bootstrap_options = "striped", full_width = F, position = "center",latex_options = "HOLD_position")
@

<<Timecompareweek2Cpu, cache=TRUE, eval=TRUE>>=
#using CPU
...
<<Timecompareweek2Gpu, eval=TRUE, cache=FALSE>>=
#using GPU
...
system.time(gpuRandom::fisher.sim(week_GPU, 1e7, streams=streams,
            type="double", returnStatistics=FALSE,Nglobal = Nglobal))
@
\fct{stats::fisher.test} takes 60.4 seconds, while \fct{fisher.sim} takes about 10.8 seconds, the elapsed time is decreased to about 20\% of the time taken on CPU. 
<<Timecompareweek2GpuTest, eval=TRUE, cache=TRUE>>=
...
@
With about ten million simulations, we get 1272 cases and a p-value around 0.0001273. We summarized the results in Table \ref{tab:summary}.
<<Timecompareweek3GpuTest, echo=FALSE, eval=TRUE, cache=TRUE>>=
result_weekgpu$cpu = as.vector(result_weekgpu$sim)
@
\begin{table}[H]
\begin{tabular}{ |c|c|c|c|c|c| } 
 \hline
 test example & B & p-value stats & p-value gpuRandom & time stats & time gpuRandom \\ \hline
 Month & 1e6 & 0.4029 & 0.4035 & 9.8 & 2.3\\ \hline
 Week & 1e7 & 0.000119 & 0.000127 & 60.4 & 10.8\\ \hline
\end{tabular}
\caption{\label{tab:summary} Comparison summary of \fct{gpuRandom::fisher.sim} and \fct{stats::fisher.test}.}
\end{table}
...
@
\caption{\label{fig4} Frequency distribution for test statistics from month and week data tables.}
\end{figure}
...
gpuRandom::maternBatch(maternCov, coordsGpu, paramsGpu,
                       Nglobal=c(64,64,16), Nlocal=c(4,4,4))
@
...
<<r plotSimSetup, echo=TRUE, eval=TRUE, cache=TRUE>>=
library(raster)
...
\begin{figure}[H]
\centering
<<maternplot, echo=TRUE, fig.show='hold', eval=TRUE, dev='pdf',cache=TRUE>>=
par(mar=rep(0.1, 4))
