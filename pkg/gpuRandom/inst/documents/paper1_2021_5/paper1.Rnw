...
library("clrng")
myStreamsCpu <- createStreamsCpu(n=4, initial=rep(12345, 6))
t(myStreamsCpu)
@
...
@
Equivalently, 
 \fct{createStreamsGpu} creates streams directly on the GPU and returns a `\code{vclMatrix}', which makes it slightly more efficient when the number of streams is large.
<<createStreams_Gpu2>>=
myStreamsGpu2 = createStreamsGpu(n=4, initial=rep(12345, 6)))
@
