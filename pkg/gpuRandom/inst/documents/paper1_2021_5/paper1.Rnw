...
@
The ``week'' table has a much smaller p-value: around $\Sexpr{sn(result_weekcpu$p.value,3)}$, which should require a larger number of simulations to get a more accurate p-value. With more than ten million simulations, we get 1350 cases and a p-value around $\Sexpr{sn(resultWeek$p.value,3)}$ with \fct{fisher.sim}. \fct{stats::fisher.test} takes about 91.4 seconds, while \fct{fisher.sim} takes about 1.9 seconds, the GPU elapsed time is decreased to about 2.1\% of the time taken on CPU. 
...
dt <- data.frame(
  ...
  var4 = c('Intel 3.7ghz', 0.4035606,0.0001202, 14.8,  91.4),
