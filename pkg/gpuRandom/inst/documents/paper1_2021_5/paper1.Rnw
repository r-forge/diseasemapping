...
%
\begin{framed}
\begin{lstlisting}[language=C,basicstyle=\small,label={lst:normal},caption={Using streams to produce standard normal random numbers},escapeinside={(*@}{@*)}]
__kernel void mrg31k3pMatrix(
  __global int* streams,
  __global float* out){
...
(*@{\hspace*{1cm}\vdots}@*)
local float part[2];
float sinOrCosPart1, addForSine = get_local_id(1) * PI;
if(get_local_id(1)){
...
for(DrowBlock = 0, Drow=get_global_id(0), DrowStart = Drow * NpadCol;
    ...
    for(DcolBlock = 0, Dcol=get_global_id(1), Dentry = DrowStart + Dcol;
      ...
      // cos for local0, sine for local1
