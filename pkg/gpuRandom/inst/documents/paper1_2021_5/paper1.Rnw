...
myStreamsCpu <- createStreamsCpu(n=4)
t(myStreamsCpu)
@
...
\end{itemize}
For the MRG31k3p RNG, each state of the stream is comprised of six 31-bit integers, the state of the first stream (seed of the \proglang{clRNG} library) is by default ``12345''. By setting a seed same as the MRG31k3p default seed in \fct{createStreamsGpu}, we can produce on device exactly the same streams as those created on host as follows,
<<createStreams_Gpu>>=
# creating streams on gpu
#myStreamsGpu1 = vclMatrix(myStreamsCpu)
myStreamsGpu1 = createStreamsGpu(n=4, seed=12345)
range(as.matrix(myStreamsGpu1) - myStreamsCpu)
matrix(as.matrix(myStreamsGpu1), nrow(myStreamsCpu), ncol(myStreamsCpu), dimnames = dimnames(myStreamsCpu))
@
\fct{createStreamsGpu} also has the two arguments \code{n}, which is the number of streams to create, and \code{seed}, which is the seed of streams, and is also by default a vector of six ``12345''. \fct{createStreamsGpu} is similar to \code{.Random.seed} in \proglang{R}. By setting the RNG status with the \proglang{R} function \fct{set.seed}, we are able to repeat the sequence of random seeds, which is an important quality criteria of RNGs. Similarly in \fct{createStreamsGpu} and \fct{createStreamsCpu}, with the \code{seed} argument, we are able to produce exactly the same sequence of streams as many times as we want.
...
\begin{itemize}
  ...
  \item \code{Nglobal}: global index space (NDRange). Default is set as $(64,8)$.
  \item \code{type}: ``double'' or ``float'' format of generated random numbers.
\end{itemize}
Next we show that by using a stream \code{myStreamsGpu2} same as \code{myStreamsGpu1},  \fct{runif} returns a vector \code{sim_2} of same random numbers as \code{sim_1}. 
<<eval=TRUE,tidy=TRUE>>=
sim_2 <-runif(n=6, streams=myStreamsGpu2, Nglobal=c(1,4), type = "double")
as.vector(sim_2)
@
Notice that \code{myStreamsGpu2} is a \code{deepcopy} of \code{myStreamsGpu1} before \code{myStreamsGpu1} is used. Reusing \code{myStreamsGpu1} will produce a vector different from \code{sim_1} (see \code{sim_3}). Because each time a random number is generated, the current state of the stream is changed (advances by one position), below shows the first six rows (current states) of \code{myStreamsGpu1} after it being used.
<<eval=TRUE,tidy=TRUE>>=
sim_3 = runif(n=6,streams=myStreamsGpu1, Nglobal=c(1,4), type="double")
as.vector(sim_3)
t(as.matrix(myStreamsGpu1))[1:6,]
@
...
sim_saved = runif(n=6,streams=streams_saved, Nglobal=c(1,4), type="double")
as.vector(sim_saved)
@
