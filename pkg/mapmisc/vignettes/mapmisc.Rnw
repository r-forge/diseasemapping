...
if(!exists('mapmiscCachePath'))
  mapmiscCachePath = system.file('extdata', package='mapmiscSp')
...
<<meuseProjNotRun,eval=FALSE,echo=TRUE, purl=FALSE>>=
library('rgdal')
...
<<elevation,eval=FALSE, purl=FALSE>>=
library('raster')
...
Some of the code in the  following sections require the {\tt rgdal} package to
run.
